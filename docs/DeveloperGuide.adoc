...
* **Alternative 3 (current implementation):** Store the search results in a `List` ordered by their <<levenshtein-distance,Levenshtein distances>> from the search keyword, given that
the names/tags contain the keyword.
** Pros: Searches are ordered by a degree of similarity, instead of the random order of names/tags in Alternative 2.
** Cons: Added complexities in finding.
{zwsp}
